"""Dev-only: capture real decrease-loan (repay) txs from dbsync into JSON fixtures.

Captures both a FULL repay (loan token + owner NFT burned, no loan output) and a
PARTIAL repay (loan output kept open). Each fixture embeds enough to (a) byte-exact
test the redeemer + pool/loan datums and (b) forward-replay against the spent inputs
via Ogmios ``additionalUtxo``: full tx CBOR, every resolved input (pool, loan,
borrower) with address/value/inline datum, reference inputs, the decoded redeemers,
the pool_in/pool_out + loan_in/loan_out datums, mint entries, and a ``realized`` block
of computed deltas.

Run: PYTHONPATH=src python tests/lending/danogo/fixtures/_capture_decrease_loan.py
...
# Loan validator script hash: the policy of the loan/owner tokens and the credential
# of the Withdraw(loan_skh) delegation hub that orchestrates DecreaseLoanAmount.
LOAN_SKH = "aca8e306eda3eb6c25a838bebac37d929c216aab13c8d463fca5a08d"

# The constructor index of DecreaseLoanAmount in the loan validator's redeemer union
# (CBOR tag 126 == 0xd87e). The Withdraw(loan_skh) reward redeemer carrying it is the
# selector that distinguishes repay txs from every other loan action.
DECREASE_LOAN_CONSTR_TAG = b"\xd8\x7e"

# Chosen mainnet repay transactions (clean, few-input, recent). Overridable via env.
FULL_TX = os.environ.get(
    "DECREASE_LOAN_FULL_TX",
    "376adb3a76c61d85b2a625bb3368f77313bd83ef03188a6abc8b5e5b2feb1874",
)
PARTIAL_TX = os.environ.get(
    "DECREASE_LOAN_PARTIAL_TX",
    "e1bbfb7b973f2b265f9936ab77f1eb98fb50aeff44938232701f526404f1fde5",
)
# Partial repays that ALSO modify the loan's locked collateral (its multi-asset
# holdings), so the loan output's collateral differs from the loan input's. REMOVE
# returns collateral to the borrower; ADD locks more collateral. Both still ride the
# same DecreaseLoanAmount redeemer (collateral lives in the UTxO value, not the datum).
REMOVE_COLLAT_TX = os.environ.get(
    "DECREASE_LOAN_REMOVE_COLLAT_TX",
    "77f13e237e1fad0a3b7cf04ad0c5a57f0c8d9ca88cff3f65bb6843a5b43e4f9c",
)
ADD_COLLAT_TX = os.environ.get(
    "DECREASE_LOAN_ADD_COLLAT_TX",
    "5bbe7998303bc1aed6d62708d7ee5d1d34135c3c4ad6cda9fcfd56d692a9d1ae",
)
...
def _decode_redeemer(cbor_hex: str) -> dict:
    """Decode a DecreaseLoanAmount redeemer (constr 5) into its named fields."""
    tag = cbor2.loads(bytes.fromhex(cbor_hex))
    assert isinstance(tag, cbor2.CBORTag) and tag.tag == 126, "expected constr 5"
    ...
    return dict(
        ...
        loan_out_idx=_opt(loan_out_idx),
...
def _collateral_delta(
    ...
    *,
    ...
    as opposed to a same-unit quantity change of an already-locked collateral).
    """
...
def _realized(
    *,
    ...
    """Compute the on-chain repay deltas (byte-exact targets for the builder).
    ...
    - ``pool_change_amount = total_borrow_in + new_accumulated_interest -
      total_borrow_out`` (== ``current_loan_amount`` on full repay).
    - ``pool_out.total_supply = total_supply_in + new_accumulated_interest -
      new_loan_interest_fee`` (NOT raised by the repayment).
    - The repay SWEEPS fees: ``pool_out.undistributed_fee == 0`` and the fee output
      receives ``undistributed_fee_in + new_loan_interest_fee`` supply tokens.
    - The pool's raw supply-token holdings rise by ``pool_change_amount -
      fee_output_amount`` (the rest of the repayment goes to the fee output).
    """
    ...
    current_loan_amount = floor_div(
        loan_in.loan_amount * pool_out.interest_index,
        loan_in.initial_interest_index,
    )
    pool_change_amount = (
        pool_in.total_borrow + new_accumulated_interest - pool_out.total_borrow
    )
    # Cross-checks against the captured datum/value (raise if the model drifts).
    assert fee_output_amount == pool_in.undistributed_fee + new_loan_interest_fee
    assert pool_out.undistributed_fee == 0
    assert pool_out_supply - pool_in_supply == pool_change_amount - fee_output_amount
    ...
    return dict(
        ...
        total_supply_before=pool_out.total_supply,
        prev_undistributed_fee=pool_in.undistributed_fee,
        pool_out_undistributed_fee=pool_out.undistributed_fee,
...
def capture(tx_hash: str, *, variant: str) -> dict:
    ...
    # Loan input (parse LoanDatum); loan output present only on partial repay.
    loan_in = next(u for u in inputs if _parses(u["datum"], LoanDatum))
    loan_out_entry = next(
        (u for u in outputs.values() if _parses(u["datum"], LoanDatum)),
        None,
    )
    ...
    decrease_cbor = next(
        r["cbor"]
        for r in redeemers
        if r["purpose"] == "reward" and r["script_hash"] == loan_skh
    )
    oracle_cbor = next(
        (
            r["cbor"]
    ...
    decoded = _decode_redeemer(decrease_cbor)
    ...
    loan_out_datum_obj = (
        LoanDatum.from_cbor(loan_out_entry["datum"]) if loan_out_entry else None
    )
    # Collateral lives in the loan UTxO VALUE; compare loan_in vs loan_out holdings
    # (present only on a partial repay) to surface any collateral modification.
    collateral = (
        _collateral_delta(
            loan_in,
            loan_out_entry,
        ...
        if loan_out_entry is not None
        else None
    )
    fee_output = outputs[decoded["fee_out_idx"]]
    realized = _realized(
        ...
        fee_output_amount=_supply_qty(fee_output, supply_token),
    )
    # Embed the collateral target/delta in the realized block: downstream byte-exact
    # tests build the loan output against ``collateral_out`` (the target absolute map).
    ...
    return dict(
        ...
        decrease_loan_redeemer=decrease_cbor,
        decrease_loan_redeemer_decoded=decoded,
        ...
        loan_out_datum=loan_out_entry["datum"] if loan_out_entry else None,
        loan_in_amount=loan_in_datum_obj.loan_amount,
        loan_in_initial_index=loan_in_datum_obj.initial_interest_index,
        loan_out_amount=(
            loan_out_datum_obj.loan_amount if loan_out_datum_obj else None
        ),
        loan_out_initial_index=(
            loan_out_datum_obj.initial_interest_index if loan_out_datum_obj else None
        ),
...
if __name__ == "__main__":
    ...
    full = capture(FULL_TX, variant="full")
    (here / "decrease_loan_tx.json").write_text(
        json.dumps(full, indent=2, sort_keys=True) + "\n"
    )
    print(f"wrote decrease_loan_tx.json (full, tx {full['tx_hash']})")

    partial = capture(PARTIAL_TX, variant="partial")
    (here / "decrease_loan_partial_tx.json").write_text(
        json.dumps(partial, indent=2, sort_keys=True) + "\n"
    )
    print(f"wrote decrease_loan_partial_tx.json (partial, tx {partial['tx_hash']})")

    # Partial repays that ALSO modify the loan's locked collateral.
    remove_collat = capture(REMOVE_COLLAT_TX, variant="partial")
    (here / "decrease_loan_remove_collat_tx.json").write_text(
        json.dumps(remove_collat, indent=2, sort_keys=True) + "\n"
    )
    print(
        "wrote decrease_loan_remove_collat_tx.json "
        f"(partial+remove, tx {remove_collat['tx_hash']}, "
        f"delta {remove_collat['collateral']['collateral_delta']})"
    )

    add_collat = capture(ADD_COLLAT_TX, variant="partial")
    (here / "decrease_loan_add_collat_tx.json").write_text(
        json.dumps(add_collat, indent=2, sort_keys=True) + "\n"
    )
    print(
        "wrote decrease_loan_add_collat_tx.json "
        f"(partial+add, tx {add_collat['tx_hash']}, "
        f"delta {add_collat['collateral']['collateral_delta']})"
    )
